...
- via option _go_out_, the resultant __<def>.pb.go__ output file will be placed at the same directory retaive to input source, via option _go_opt_; and
- via option _go-grpc_out_, the resultant __<def>_grpc.pb.go__ output file will be placed at the same directory retaive to input source, via option _go-grpc_opt_;
